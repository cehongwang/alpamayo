...
# Built from: https://github.com/chohk88/TensorRT-Edge-LLM/tree/feature/torch-tensorrt-python-runtime
DEFAULT_PLUGIN_PATH = "/home/TensorRT-Edge-LLM/build/libNvInfer_edgellm_plugin.so"
...
def _register_plugin_op_impl() -> None:
    ...
    The TensorRT-Edge-LLM plugin (0.4.0-based) requires 5 inputs:
    ...
    With enable_delta_kv_output=1, output KV shape is [B, 2, H, SeqLen, D] (delta only).
    """
    ...
    def attn(
        ...
        # Delta KV output: shape is [B, 2, H, SeqLen, D]
        updated_kv = torch.zeros(
            batch_size, 2, nkv, seq_len, d, dtype=qkv.dtype, device=qkv.device
        )
...
if not (
    hasattr(torch.ops, "tensorrt_edge_llm")
    and hasattr(torch.ops.tensorrt_edge_llm, "xqa_attn")
):
    _register_plugin_op_impl()
